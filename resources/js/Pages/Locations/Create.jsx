...
export default function Create({auth}) {
    ...
    return (
        <AuthenticatedLayout
            user={auth.user}
            header={<h2 className='text-xl font-semibold leading-tight text-gray-800'>Add New Location</h2>}
        >
            ...
            <Container>
                <Card title={'Add Location'}>
                    <form onSubmit={handleSubmit}>
                        ...
                        <Editor
                            apiKey="babsfkde0ygouwy8ntz8fsci0iqdgz10swomq85qgpzo13rh" // Untuk sementara saya pake apiKey dari mas "ahmad hafidh" dulu, tapi nanti saya ganti dgn punya saya dgn cara daftar di https://www.tiny.cloud/
                        ...
                        <select value={data.category_id} onChange={e => setData('category_id', e.target.value)} className="w-full p-2 mb-4 border rounded-sm">
                            <option value="">Select Category</option>
                            {categories.map((category) => (
                                <option key={category.id} value={category.id}>{category.name}</option>
                            ))}
                        </select>
                        {errors.category_id && <div className="mt-1 text-sm text-red-500">{errors.category_id}</div>}

                        {/* Region Select */}
                        <label className="block">Region</label>
                        <select value={data.region_id} onChange={e =>setData('region_id', e.target.value)} className="w-full p-2 mb-4 border rounded-sm">
                            <option value="">Select Region</option>
                            {regions.map((region) => (
                                <option key={region.id} value={region.id}>{region.name}</option>
                            ))}
                        </select>
                        {errors.region_id && <div className="mt-1 text-sm text-red-500">{errors.region_id}</div>}
                        ...
                        {Object.entries(tickets).map(
                            ([category, ticketList]) => (
                                <div key={category} className="mb-4">
                                    ...
                                    <select value={data.ticket_ids[category] || ""} onChange={(e) => setData("ticket_ids", {...data.ticket_ids, [category]: e.target.value,})}
